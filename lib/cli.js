...
main();
async function main() {
  if (!getCommand().content[0]) {
    return showHelp();
  }
  if (getCommand().content[0] === "init") {
    return initConfig();
  }
...
function showHelp() {
  console.log(
    `
     gd init 初始化配置文件
     gd [dev/demo/prod...]
     详情请查看 https://www.npmjs.com/package/go-deploy
     `
  );
}
...
async function initConfig() {
  const exists = await fs.existsSync(path.join(process.cwd(), configFilename));
  if (exists) {
    return console.log(configFilename + " 配置文件已存在，请先手动删除它");
  }
  const template = await fs.readFileSync(path.join(__dirname, configFilename));
  await fs.writeFileSync(path.join(process.cwd(), configFilename), template);
...
async function readConfig() {
  try {
    ...
    if (getCommand().operate["config"]) {
      // 指定config文件
      config = await require(getCommand().operate["config"]);
    } else {
      // 默认执行目录下的config文件
      config = await require(path.join(process.cwd(), configFilename));
    }
